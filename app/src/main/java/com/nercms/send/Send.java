...
//    public native int DestroyEncoder(long encoder); //销毁编码器，释放资源
    ...
    //接口函数
    public static long CreateEncoder(int width, int height){
        return 1;
    }

    //编码一帧图像，返回包的数目
    //type=编码帧的类型，frame=原始yuv图像，stream=原始图像码流，packetSize=包的尺寸
    public static int EncoderOneFrame(long encoder, int type, byte[] frame, byte[] stream, int[] packetSize){
        return 1;
    }

    public static int DestroyEncoder(long encoder){
        return 1;
    }
